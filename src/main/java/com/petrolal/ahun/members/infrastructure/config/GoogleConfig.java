...
public class GoogleConfig {
  ...
  public void testCredentials() throws IOException {
      ...
      try {
        ...
        if (credentials == null) {
          throw new IllegalStateException(
              "Both google.credentials and Application Default Credentials are missing");
        }
        return;
      } catch (IOException e) {
        throw new IllegalStateException(
            "Google credentials environment variable is empty and Application Default Credentials"
                + " are not available",
            e);
      }
    }
    ...
    try (ByteArrayInputStream in =
      ...
      if (credentials == null) {
        throw new IllegalStateException(
            "Failed to parse Google credentials from environment variable");
      }
    }
  }
}
