...
class TelegramUseCasesTest {
    ...
    @BeforeEach
    void setUp() {
        telegramUseCases = new TelegramUseCases(telegramAdapter, memberPort);
    }
    ...
    void shouldSendMonthlyMessage() {
        ...
        when(telegramAdapter.sendMessage(eq(telegramAdapter), any(String.class))).thenReturn(responseDto);
        ...
        verify(telegramAdapter).sendMessage(eq(telegramAdapter), messageCaptor.capture());
