...
class MemberSpringRepositoryIT {
  ...
  @Autowired private MemberSpringRepository repository;
  ...
  void shouldFindByMonthBirthday() {
    ...
    jdbcTemplate.execute(
        String.format(
            "INSERT INTO members (uuid, member_name, email, birthday, created_at, month_birthday)"
                + " VALUES ('%s', 'John Doe', 'john@example.com', '1990-06-15', CURRENT_TIMESTAMP,"
                + " 6)",
            uuid1));
    ...
    jdbcTemplate.execute(
        String.format(
            "INSERT INTO members (uuid, member_name, email, birthday, created_at, month_birthday)"
                + " VALUES ('%s', 'Alice Smith', 'alice@example.com', '1995-07-20',"
                + " CURRENT_TIMESTAMP, 7)",
            uuid2));
