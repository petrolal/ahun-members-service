...
class GoogleSheetWebResourceIT {
  ...
  @MockBean private GoogleSheetPort googleSheetPort;

  @Test
  void shouldReadMemberSheet() throws Exception {
    MemberFromSheetDto dto =
        new MemberFromSheetDto(
            LocalDateTime.now(), "jane@example.com", "Jane Doe", LocalDate.of(1990, 5, 15));
    when(googleSheetPort.readMemberSheet()).thenReturn(List.of(dto));

    mockMvc
        .perform(get("/api/sheets").contentType(MediaType.APPLICATION_JSON))
  ...
  void shouldSyncSheet() throws Exception {
    ...
    mockMvc
        .perform(post("/api/sheets").contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
