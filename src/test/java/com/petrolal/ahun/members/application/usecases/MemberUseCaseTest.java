...
class MemberUseCaseTest {
  ...
  @Mock private MemberRepositoryPort memberRepositoryPort;
  ...
  void shouldGetMembersByCurrentMonth() {
    Member member =
        new Member(
            ...
            "Alice",
            "alice@example.com",
            LocalDate.of(1995, LocalDate.now().getMonthValue(), 10),
            LocalDateTime.now());
  ...
  void shouldGetBirthdaysByMonthAndDate() {
    ...
    Member member =
        new Member(
            ...
            "Alice",
            "alice@example.com",
            LocalDate.of(1995, today.getMonthValue(), today.getDayOfMonth()),
            LocalDateTime.now());
    when(memberRepositoryPort.findByMonthAndDay(today.getMonthValue(), today.getDayOfMonth()))
